...
# 2. The benchmark table is split into a visual table and a status table.
def test_readme_has_two_benchmark_tables() -> None:
    assert "### 1. Visual" in README
    assert "### 2. Engineering status" in README
