...
class OpenEMSSolver(RFSolver):
    ...
    def solve(self, meshed: SolverOutput, output_dir: Path) -> SolverOutput:
        ...
        try:
            result = subprocess.run(
                [ems_bin, str(xml_file), f"--numThreads=4"],
          ...
          <ContinuousStructure CoordSystem="0">
            <Properties>
              <Material Name="substrate">
                ...
                <Primitives>
                  <Box P1="{-(w/2+s+20):.3f} {-50:.3f} {-h:.3f}" P2="{w/2+s+20:.3f} {l+50:.3f} 0" />
                </Primitives>
              </Material>
              <Metal Name="center_conductor">
                <Primitives>
                  <Box P1="{-w/2:.3f} 0 0" P2="{w/2:.3f} {l:.3f} {t:.4f}" />
                </Primitives>
              </Metal>
              <Metal Name="ground_left">
                <Primitives>
                  <Box P1="{-(w/2+s+20):.3f} 0 0" P2="{-(w/2+s):.3f} {l:.3f} {t:.4f}" />
                </Primitives>
              </Metal>
              <Metal Name="ground_right">
                <Primitives>
                  <Box P1="{w/2+s:.3f} 0 0" P2="{w/2+s+20:.3f} {l:.3f} {t:.4f}" />
                </Primitives>
              </Metal>
            </Properties>
            <RectilinearGrid>
              <XLines>{_linspace(-(w/2+s+25), w/2+s+25, int((w+2*s+50)/cell+2))}</XLines>
              <YLines>{_linspace(-30, l+30, max(int((l+60)/cell+2), 50))}</YLines>
