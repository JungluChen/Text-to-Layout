...
from textlayout._legacy.pdk import SuperconductingPDK, load_pdk
...
def community_device_library() -> dict[str, Any]:
    from textlayout._legacy.layout_automation import quantum_cell_library
...
def generate_experiment_notebook(title: str, experiment: dict[str, Any]) -> dict[str, Any]:
    from textlayout._legacy.platform_extensions import tutorial_notebook
...
def generate_project_template(kind: str, *, name: str) -> dict[str, str]:
    ...
    files = {
        ...
        "design.py": f"from textlayout._legacy.pcells import {kinds[kind]}\n\ncomponent = {kinds[kind]}()\n",
