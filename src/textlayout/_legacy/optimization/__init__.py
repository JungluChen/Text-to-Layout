...
from textlayout._legacy.optimization.repair import (
    run_physics_repair,
    RepairResult,
    load_process_yaml,
)
...
def optimize_ljpa_parameters(
    *,
    ...
    """Deterministic local surrogate optimization for first-pass LJPA geometry."""
    parameters = {
        "cpw_length": 210.0, "cpw_trace_width": 10.0, "cpw_gap": 6.0,
        "junction_width": 0.22, "junction_height": 0.22,
        "flux_line_length": 120.0, "flux_line_width": 1.5,
        "inductor_segment_length": 24.0, "inductor_trace_width": 1.0, "inductor_pitch": 3.0,
    }
    parameters.update(initial_parameters or {})
    history: list[_OptimizationState] = []
    for iteration in range(max_iterations):
        ...
        history.append(_OptimizationState(
            iteration=iteration, parameters=dict(parameters), metrics=metrics, errors=errors,
        ))
        freq_ratio = metrics["center_frequency_ghz"] / max(target_frequency_ghz, 1e-9)
        parameters["cpw_length"] = max(20.0, min(5000.0, parameters["cpw_length"] * freq_ratio ** 2))
        bw_ratio = target_bandwidth_mhz / max(metrics["bandwidth_mhz"], 1e-9)
        parameters["cpw_gap"] = max(1.0, min(30.0, parameters["cpw_gap"] * bw_ratio ** 0.5))
        gain_error = target_gain_db - metrics["gain_db"]
        area_scale = max(0.5, min(1.8, 1.0 - gain_error / 80.0))
    ...
    history.append(_OptimizationState(
        iteration=max_iterations, parameters=dict(parameters), metrics=final_metrics, errors=final_errors,
    ))
    return {
        ...
        "notes": [
            "Deterministic local surrogate loop for first-pass geometry.",
