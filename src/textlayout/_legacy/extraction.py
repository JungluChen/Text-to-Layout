...
from textlayout._legacy.process import DEFAULT_PROCESS, Layer, layer_to_list
...
def _junction_overlap_area_um2(gds_path: Path) -> float:
    ...
    overlap = m1_region & m2_region
    if not jj_region.is_empty():
        overlap &= jj_region
...
def extract_physical_parameters(
    ...
    *,
    ...
    """
    from textlayout._legacy.extraction_schema import empty_extraction_v1, SCHEMA_VERSION
    ...
    if "JJ" in layer_groups and layer_groups["JJ"].get("area_um2", 0.0) > 0.0:
        try:
            ...
            area_um2 = 0.0
            errors.append(f"junction boolean overlap extraction failed: {exc}")

        min_area = _MIN_JJ_WIDTH * _MIN_JJ_HEIGHT
        if area_um2 <= 0.0:
            errors.append("junction area requires nonzero M1/M2 boolean overlap")
        elif area_um2 <= min_area:
            errors.append(
            ...
            result["lineage"]["junction.area"] = quantity(
                ...
                method_label="geometry_extracted",
                source="GDS",
                formula="area(M1 intersect M2 within JJ process window)",
                confidence=1.0,
            )
