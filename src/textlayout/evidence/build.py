...
def _git_commit(root: Path, paths: list[str]) -> str | None:
    """The commit that last changed this evidence's own files.

    Deliberately NOT `HEAD`. Recording HEAD would make every canonical record
    stale the moment any unrelated commit landed, so `--check` could never pass
    and the "regeneration is deterministic" guarantee would be a lie. What the
    record needs is the provenance of *its* inputs and outputs.
    """
    if not paths:
        return None
    try:
...
def build_canonical(
    ...
    common: dict[str, Any] = {
        ...
        "git_commit": _git_commit(
            repo_root,
            [
                str((showcase / name).relative_to(repo_root)).replace("\\", "/")
                for name in ("layout.json", *recipe.output_files, *recipe.input_files)
