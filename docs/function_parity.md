...
| Research adapter boundaries | `list_research_integrations` distinguishes executed adapters from experimental handoffs. scqubits, Quantum Metal/Qiskit Metal, and SQDMetal are non-passing placeholders until real component-specific execution, imported artifacts, and convergence tests exist. Historical legacy demonstrations are not product execution evidence. |
